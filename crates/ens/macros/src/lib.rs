...
use ens_macro_utils::{derive_label, ensure_no_collision, get_struct_fields, EnsManifest};
...
pub fn derive_bundle(input: TokenStream) -> TokenStream {
        ...
        .zip(field.iter())
    {
        match field_kind {
            BundleFieldKind::Component => {
                ...
                <#field_type as #ens_path::bundle::Bundle>::component_ids(components, storages, &mut *ids);
                });
                match field {
                    Some(field) => {
                        ...
                        field_from_components.push(quote! {
                            #field: <#field_type as #ens_path::bundle::Bundle>::from_components(ctx, &mut *func),
                        });
                    }
                    None => {
                        ...
                        field_from_components.push(quote! {
                            #index: <#field_type as #ens_path::bundle::Bundle>::from_components(ctx, &mut *func),
                        });
                    }
                }
            }
    ...
    TokenStream::from(quote! {
        ...
        unsafe impl #impl_generics #ens_path::bundle::Bundle for #struct_name #ty_generics #where_clause {
            fn component_ids(
                components: &mut #ens_path::component::Components,
                storages: &mut #ens_path::storage::Storages,
                ids: &mut impl FnMut(#ens_path::component::ComponentId)
            ){
                #(#field_component_ids)*
            }
            ...
            where
                __F: FnMut(&mut __T) -> #ens_path::ptr::OwningPtr<'_>
            {
                Self{
        ...
        impl #impl_generics #ens_path::bundle::DynamicBundle for #struct_name #ty_generics #where_clause {
            #[allow(unused_variables)]
            #[inline]
            fn get_components(
                self,
                func: &mut impl FnMut(#ens_path::component::StorageType, #ens_path::ptr::OwningPtr<'_>)
            ) {
...
pub(crate) fn ens_path() -> syn::Path {
