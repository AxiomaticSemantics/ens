...
impl Default for World {
    fn default() -> Self {
        Self {
            id: WorldId::new().expect("More `Ens` `World`s have been created than is supported"),
...
impl World {
    ...
    ///
    /// ```
    /// use ens::{bundle::Bundle, component::Component, world::World};
    ///
    /// #[derive(Component)]
    /// struct Position {
    ///   x: f32,
    ...
    ///
    /// ```
    /// use ens::{component::Component, entity::Entity, world::World};
    ///
    /// #[derive(Component)]
    ...
    ///
    /// When using `ens` as part of the full Bevy engine, this method is added as a system to the
    /// main app, to run during `Last`, so you don't need to call it manually. When using `ens`
    ...
    /// run queries on the [`World`] by storing and reusing the [`QueryState`].
    /// ```
    /// use ens::{component::Component, entity::Entity, world::World, query::With};
    ///
    /// #[derive(Component)]
    /// struct A;
    /// #[derive(Component)]
    ...
    ///
    /// ```
    /// use ens::{entity::Entity, world::World, component::Component};
    /// #[derive(Component)]
    /// struct A(&'static str);
    /// #[derive(Component, PartialEq, Debug)]
    /// struct B(f32);
