...
impl EnsManifest {
    ...
    /// the [manifest](EnsManifest) by [name](str).
    pub fn maybe_get_path(&self, name: &str) -> Option<syn::Path> {
        ...
        let find_in_deps = |deps: &Item| -> Option<syn::Path> {
            ...
            if let Some(module) = name.strip_prefix("ens_") {
    ...
    /// Attempt to get a subcrate [path](syn::Path) under Ens by [name](str)
    pub fn get_subcrate(&self, subcrate: &str) -> Option<syn::Path> {
        self.maybe_get_path(ENS)
            .map(|ens_path| {
                let mut segments = ens_path.segments;
                segments.push(EnsManifest::parse_str(subcrate));
                syn::Path {
                    leading_colon: None,
                    segments,
                }
            })
            .or_else(|| self.maybe_get_path(&format!("ens_{subcrate}")))
    }
}
